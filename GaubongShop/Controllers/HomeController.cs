...
namespace GaubongShop.Controllers
{
    public class HomeController : Controller
    {
        ...
        public ActionResult ProductDetails(int?id, int?quantity, int? page)
        {
            ...
            model.product = pro;
            model.RealatedProducts = (System.Collections.Generic.List<Product>)products.OrderBy(p => p.ProductID).Take(3).ToPagedList(pageNumber, pageSize);
            model.TopProducts = products.OrderByDescending(p => p.OrderDetails.Count()).Take(2).ToPagedList(pageNumber, pageSize);
